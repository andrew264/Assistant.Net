    ...
    ILogger<GeniusLyricsService> logger)
{
    ...
    {
        if (string.IsNullOrWhiteSpace(externalApiOptions.Value.GeniusToken))
        {
        ...
        request.Headers.Authorization = new AuthenticationHeaderValue("Bearer", externalApiOptions.Value.GeniusToken);
